...
async fn resolved_database_kv_success_shapes() -> TestResult {
    ...
    ) && let Some(payload) = record_kv_response(
        &mut failures,
        "kv_get missing local key",
        &result,
        "local",
        &["found", "resolved_database", "value"],
    ) && (payload["found"] != serde_json::json!(false)
    ...
    ) && (payload["count"] != serde_json::json!(0) || payload["stores"] != serde_json::json!([]))
    {
        failures.push(format!(
    ...
    ) && (payload["deleted"] != serde_json::json!(false)
        || payload["store"] != serde_json::json!("empty_local")
    ...
    ) && (payload["removed"] != serde_json::json!(0)
        || payload["store"] != serde_json::json!("empty_local"))
    {
        failures.push(format!(
    ...
    ) && (payload["stored"] != serde_json::json!(true)
    ...
    if let Some(result) = call_case!(
        ...
        "persistent",
        &["found", "resolved_database", "value"],
    ) && (payload["found"] != serde_json::json!(true)
    ...
    ) && (payload["stored"] != serde_json::json!(false)
        || payload["created"] != serde_json::json!(false)
    ...
    ) && (payload["stored"] != serde_json::json!(2)
    ...
    ) && (payload["stored"] != serde_json::json!(1)
        || payload["created"] != serde_json::json!(1)
    ...
    if let Some(result) = call_case!(
        ...
        "mixed_kv",
        &["count", "keys", "resolved_database", "store"],
    ) && (payload["count"] != serde_json::json!(4)
    ...
    if let Some(result) = call_case!(
        ...
        "mixed_kv",
        &["bytes", "resolved_database", "size", "store"],
    ) && (payload["size"] != serde_json::json!(4) || payload["bytes"] != serde_json::json!(11))
    {
        failures.push(format!(
    ...
    if let Some(result) = call_case!(
        ...
        "mixed_kv",
        &["removed", "resolved_database", "store"],
    ) && (payload["removed"] != serde_json::json!(2)
        || payload["store"] != serde_json::json!("routed"))
    {
        failures.push(format!(
