...
8. **Use conventional commit prefixes for changelog generation.** Do not edit `CHANGELOG.md` files by hand — release-please generates them automatically from conventional commit messages on every push to `main`. Use `feat:` for new features, `fix:` for bug fixes, and `feat!:`/`fix!:` for breaking changes. Non-user-visible work uses `chore:`, `ci:`, `docs:`, `refactor:`, or `test:` (these don't appear in the changelog). See [CONTRIBUTING.md](CONTRIBUTING.md#commit-message-format) for the full policy.
