...
| --- | --- | --- |
| **M-OOBE** | `hyperdb-api-core` requires `protoc` at build time. [`hyperdb-api-core/build.rs`](../hyperdb-api-core/build.rs) calls `tonic_prost_build::configure()`, so a published-crate consumer must install a tool beyond `cargo` and `rustc`. | Upstream is explicit that when tools are needed to generate Rust (it names `.proto` compilation as the example), the generation should run in the publishing workflow and the resulting `.rs` be vendored into the published crate. Pre-existing; tracked as follow-up work, not part of the 1.88 uplift. This is also why the RHEL compatibility job must `dnf install protobuf-compiler`. |
